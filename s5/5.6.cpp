...
// Рекурсивная функция для генерации перестановок и подсчета
int count_matches = 0;

void generatePermutations(vector<int>& balls, int start, int n) {
    if (start == n) {
        // Проверяем условие: хотя бы один шарик на своем месте
        for (int i = 0; i < n; i++) {
            if (balls[i] == i + 1) {
                count_matches++;
                return;
...
// Задача 5.6: Шарики (рекурсивное решение)
...
// Задача 5.6: Шарики (формула из задания)
...
// Задача 5.6: Шарики (тестирование малых n)
void task5_6_test() {
